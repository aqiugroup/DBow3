...
class EAPoint {
  ...
  public:
    ...
    {
        if (newX < 0)
            throw("IllegalArgumentException for x coordinate : ");
        x = newX;
    }
    ...
    {
        if (newY < 0)
            throw("IllegalArgumentException for y coordinate : ");
        y = newY;
    }
...
class ImageProcessor {
  public:
    ...
    void setFirstImgFlag(bool flag)
    {
        is_first_img = flag;
    }
    ...
    stereoCallback(const cv::Mat& cam0_img, const cv::Mat& cam1_img, double time);

    void putInGrid(const cv::Mat& img, const vector<cv::KeyPoint>& keypoints, vector<int>& inlier_markers);
  ...
  private:
     ...
     */
    void integrateImuData(cv::Mat& cam0_R_p_c, cv::Mat& cam1_R_p_c);
