...
class FastSearch {
  public:
    ...
    void                              create(Vocabulary& voc)
    {
        ...
        _child_off_start =
            _feature_off_start +
            voc.m_k * _desc_size_bytes;  // where do children information start from the start of the block
        ...
        for (const auto& Block : parent_children) {
            ...
            for (const auto& c : Block.second) {
                const auto& node = voc.m_nodes[nid_vpos[c]];
                node_info*  ptr_child =
                    (node_info*)(_data + block_offset_bytes + _child_off_start + sizeof(node_info) * idx);
            ...
            while (!done) {
                node_info* ni;  //=getBestOfBlock(block,desc.row(i));
                {
                    ...
                    for (i = 0; i < n4; i += 4) {
                        ...
                        uint32_t  d   = __builtin_popcountl(dptr[0] ^ ptr[0]) + __builtin_popcountl(dptr[1] ^ ptr[1]) +
                                     __builtin_popcountl(dptr[2] ^ ptr[2]) + __builtin_popcountl(dptr[3] ^ ptr[3]);
                        ...
                        uint32_t  d2    = __builtin_popcountl(dptr[0] ^ ptr_2[0]) +
                                      ...
                                      __builtin_popcountl(dptr[2] ^ ptr_2[2]) + __builtin_popcountl(dptr[3] ^ ptr_2[3]);
                        ...
                        uint32_t  d3    = __builtin_popcountl(dptr[0] ^ ptr_3[0]) +
                                      ...
                                      __builtin_popcountl(dptr[2] ^ ptr_3[2]) + __builtin_popcountl(dptr[3] ^ ptr_3[3]);
                        ...
                        uint32_t  d4    = __builtin_popcountl(dptr[0] ^ ptr_4[0]) +
                                      ...
                                      __builtin_popcountl(dptr[2] ^ ptr_4[2]) + __builtin_popcountl(dptr[3] ^ ptr_4[3]);
                        if (d < mind) {
                            mind    = d;
                            bestIdx = i;
                        }
    ...
    void saveToFile(const std::string& filepath) throw(std::exception)
    {
        ...
        if (!str)
            throw std::runtime_error("Vocabulary::saveToFile could not open:" + filepath);
        // magic number
        ...
        cout << "save:" << int(_data[0]) << " " << int(_data[1]) << " " << int(_data[2]) << " " << int(_data[3])
             << endl;
